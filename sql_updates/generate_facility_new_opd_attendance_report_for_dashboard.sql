...
DROP PROCEDURE IF EXISTS `generate_facility_new_opd_attendance_report_for_dashboard` $$
CREATE PROCEDURE `generate_facility_new_opd_attendance_report_for_dashboard`(INOUT message varchar(50), last_reporting_date date)
PROC:BEGIN	
	...
	CREATE OR REPLACE TABLE  `dashboard_reporting_facility_new_opd_attendances` (
		  ...
		  `book_row_number` VARCHAR(3) NULL  DEFAULT 2,
		  `row_description` VARCHAR(350) NULL DEFAULT 'Mahudhurio ya kwanza/wagonjwa wapya (kwenye kituo husika kwa tatizo fulani la kiafya)',
	...
	TRUNCATE `dashboard_reporting_facility_new_opd_attendances`;
	...
	SET @st = CONCAT("INSERT INTO dashboard_reporting_facility_new_opd_attendances(facility_code, department_code, male_under_one_month, male_under_one_year, male_under_five_year, male_above_five_under_sixty, male_above_sixty, female_under_one_month, female_under_one_year, female_under_five_year, female_above_five_under_sixty, female_above_sixty, health_plan_code, attendance_date, reporting_date) SELECT '",facility_code,"','000OPD', SUM(CASE WHEN Sex = 'MALE' AND AgeGroup < 1 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'MALE' AND AgeGroup BETWEEN 1 AND 11 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'MALE' AND AgeGroup BETWEEN 12 AND 59 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'MALE' AND AgeGroup BETWEEN 60 AND 719 THEN Cases ELSE 0 END),SUM(CASE WHEN Sex = 'MALE' AND AgeGroup >= 720 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'FEMALE' AND AgeGroup < 1 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'FEMALE' AND AgeGroup BETWEEN 1 AND 11 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'FEMALE' AND AgeGroup BETWEEN 12 AND 59 THEN Cases ELSE 0 END), SUM(CASE WHEN Sex = 'FEMALE' AND AgeGroup BETWEEN 60 AND 719 THEN Cases ELSE 0 END),SUM(CASE WHEN Sex = 'FEMALE' AND AgeGroup >= 720 THEN Cases ELSE 0 END), HealthPlanCode, CaseDate, '", last_reporting_date, "' FROM categorical_atendance GROUP BY CaseDate, HealthPlanCode");
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_under_one_month = male_under_one_month+female_under_one_month where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_under_one_year = male_under_one_year+female_under_one_year where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_under_five_year = male_under_five_year+female_under_five_year where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_above_five_under_sixty = male_above_five_under_sixty+female_above_five_under_sixty where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_above_sixty = male_above_sixty+female_above_sixty where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_male = male_under_one_month+male_under_one_year+male_under_five_year+male_above_five_under_sixty+male_above_sixty where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set total_female=female_under_one_month+female_under_one_year+female_under_five_year+female_above_five_under_sixty+female_above_sixty where facility_code='",facility_code,"'");	
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
	
	SET @st = CONCAT("UPDATE dashboard_reporting_facility_new_opd_attendances set grand_total = total_male+total_female where facility_code='", facility_code,"'");
	PREPARE stmt FROM @st;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
END$$
