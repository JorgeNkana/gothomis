...
DROP PROCEDURE IF EXISTS `generate_radiology_report_for_dashboard` $$
CREATE PROCEDURE `generate_radiology_report_for_dashboard`(INOUT message varchar(50), last_reporting_date date)
PROC:BEGIN		
	...
	IF DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY) = DATE(last_reporting_date) THEN
		TRUNCATE dashboard_reporting_radiology;
		LEAVE PROC;
	END IF;
	...
	CREATE OR REPLACE TABLE `dashboard_reporting_radiology` (
		  `concept_code` varchar(50) COLLATE utf8mb4_unicode_ci,
		  `sub_department_code` varchar(6) COLLATE utf8mb4_unicode_ci,
							...
							ELSE
								BEGIN
									IF cases > 0 AND concept_code IS NOT NULL AND TRIM(concept_code) <> '' AND health_plan_code IS NOT NULL AND TRIM(health_plan_code) <> '' THEN
										BEGIN
											SET @st = CONCAT("INSERT INTO  dashboard_reporting_radiology(facility_code, concept_code,sub_department_code, health_plan_code, number_of_investigations, test_date, reporting_date) SELECT '",facility_code,"','",concept_code,"',",sub_department_code,",",health_plan_code,",",cases, ",'",test_date,"','",last_reporting_date,"'");
											PREPARE stmt FROM @st;
											EXECUTE stmt;
											DEALLOCATE PREPARE stmt;
										END;
