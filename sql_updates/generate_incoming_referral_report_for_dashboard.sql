...
DROP PROCEDURE IF EXISTS `generate_incoming_referral_report_for_dashboard` $$
CREATE PROCEDURE `generate_incoming_referral_report_for_dashboard`(INOUT message varchar(50), last_reporting_date date)
PROC:BEGIN		
	...
	IF DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY) = DATE(last_reporting_date) THEN
		TRUNCATE dashboard_reporting_incoming_referral;
		LEAVE PROC;
	END IF;
	...
	CREATE OR REPLACE TABLE `dashboard_reporting_incoming_referral` (
		  ...
		  `row_description` VARCHAR(350) NULL DEFAULT 'Waliopewa Rufaa OPD kwenda clinic mahsusi',
