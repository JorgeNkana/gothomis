...
$facility_code=$sql[0]->facility_code;
    ...
    {
        $name = $request['name'];
        $patients = DB::table('vw_residences')
            ->distinct()
	 ...
	 public function getPatientsToEdit(Request $request)
    {
        ...
        $name = $request['name'];
		$patients="SELECT t1.* FROM tbl_patients t1 WHERE CONCAT(t1.first_name,' ',t1.middle_name,' ',t1.last_name) LIKE '%".$name."%' 
		OR t1.mobile_number LIKE '%".$name."%'
		OR t1.medical_record_number LIKE '%".$name."%'
		GROUP BY t1.medical_record_number";
    ...
    {
        $name = $request['name'];
        $patients="SELECT t1.* FROM vw_patients_search t1 WHERE CONCAT(t1.first_name,' ',t1.middle_name,' ',t1.last_name) LIKE '%".$name."%'
        OR t1.mobile_number LIKE '%".$name."%'
        OR t1.medical_record_number LIKE '%".$name."%'
        GROUP BY t1.patient_id LIMIT 20";
        return DB::SELECT($patients);
...
$registrationInfos=['gender'=>$gender,
    ...
    public function authorizeCardFromMember(Request $request){
        ...
        return ServiceManager::AuthorizeCard($CardNo,$VisitTypeID,$ReferralNo,$patient_id);
    }
    ...
    public function complete_registration(Request $request) {   // return $request->input('facility_id');
        ...
        if (0 === preg_match($mobile_pattern, $mobile_number_next_kin) AND !empty($mobile_number_next_kin)) {
            ...
            $responses[]=Tbl_patient::where('id',$patient_id)->get();
            $responses[]=Tbl_accounts_number::Where('patient_id',$patient_id)->orderBy('id','DESC')->take(1)->get();
            $responses[]=DB::table('vw_residences')->where('residence_id',$residence_id)->get();
            $responses[]=patientRegistration::getLastVisit($facility_id,$patient_id);
    ...
    public function enterEncounter(Request $request) {
		...
		if(patientRegistration::duplicate('tbl_invoice_lines',array('patient_id','item_type_id','quantity',"((timestampdiff(minute,created_at,CURRENT_TIMESTAMP) <=0))"), array($patient_id,$item_type_id,$quantity))==true){
		...
		//Melchiory: also this should have returned before creating account number
		if (!isset($service_id)) {
            return response()->json([
		...
		if(!patientRegistration::duplicate('tbl_accounts_numbers',array('patient_id',"((timestampdiff(hour,created_at,CURRENT_TIMESTAMP) <=24))"), array($patient_id))){
