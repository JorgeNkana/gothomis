...
{	
    ...
    protected $table = "tbl_trauma_physical_examms";

    protected $guarded = ["id"];
    protected $hidden  = array('pivot');
	
	public static $rules = [
        "client_id"       => "required:tbl_trauma_physical_examms",
        "concept_value"   => "required:tbl_trauma_physical_examms",
    ];
     ...
     */
    public static function isDuplicate($new, 4id = 0){
		$records = PhysicalExamination::where("id", "<>", $id)->get();
    ...
    public static function getSurvey($client_id){
		$survey = [];
		$sub_sections = TraumaConcept::where("section_code","=","PHYSICAL_EXAMINATION")->get();
									
        foreach($sub_sections as $section){
			$entries = PhysicalExamination::where("concept_id","=",$section->id)
