...
{	
    ...
    protected $table = "tbl_trauma_primary_survey";

    protected $guarded = ["id"];
    protected $hidden  = array('pivot');
	
	public static $rules = [
        "client_id"       => "required:tbl_trauma_primary_survey",
        "concept_id"       => "required:tbl_trauma_primary_survey",
    ];
	
	public static $create_rules = [
        "client_id"       => "required:tbl_trauma_primary_survey",
        "concept_id"      => "required:tbl_trauma_primary_survey",
        "concept_value"   => "required:tbl_trauma_primary_survey",
    ];
    ...
    public static function isDuplicate($new, $id = 0){
		$records = PrimarySurvey::where("id", "<>", $id)->get();
    ...
    public static function getSurvey($client_id){
		$survey = [];
		$sub_sections = TraumaConcept::where("section_code","=","PRIMARY_SURVEY")->get();
									
        foreach($sub_sections as $section){
			$entries = PrimarySurvey::where("concept_id","=",$section->id)
