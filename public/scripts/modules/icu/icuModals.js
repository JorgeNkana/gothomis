...
(function () {
    ...
    app.controller('icuModals',['$scope','$http','$rootScope','$uibModal' ,'$mdDialog',
        function ($scope,$http,$rootScope,$uibModal,$mdDialog) {
            ...
            $scope.addMedicine = function(item, patient, dawa) {
                var desired =item.dose;
                var duration =item.duration;
                var frequency =item.frequency;
                var on_hand =dawa.strength;
                if(dawa.dose_formulation !='tablets'){
                    var vehicle = 1;
                    var quantity = (desired/on_hand)*vehicle*duration*(24/frequency);

                    var netQty =  Math.round(quantity);
                }
                if(dawa.dose_formulation =='tablets'){
                    var vehicle = 1;
                    var netQty = (desired/on_hand)*vehicle*duration*(24/frequency);
                   var quantity =  Math.round(netQty);
                }
                var status_id = 1;
               ...
               // var quantity = item.dose * item.duration * 24 / item.frequency;
                ...
                if (dawa == null) {
                    ...
                    swal("Please search and select medicine!","","error");
