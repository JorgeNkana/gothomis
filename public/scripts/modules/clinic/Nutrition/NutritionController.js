...
(function () {
    ...
    app.controller('NutritionController',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
        function ($scope,$http,$rootScope,$state,$uibModal,$window,$mdDialog) {
            ...
            $scope.getPerformance = function (item) {
                ...
                $http.post('/api/nutritionistPerformance',perfData).then(function (data) {
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        $scope.getPatientReport = function(item) {
                            ...
                            $http.post('/api/NutritionHistory', {

                                "patient_id": item.patient_id,

                                "visit_id": item.account_id
                        ...
                        $scope.Save_nutritional_consultations = function(item, patient) {
                            if(item==undefined){
                                swal('','Please Fill All required Field','error');
                                return;
                            } if(item.action_taken==undefined){
                                swal('','Please Choose Action Taken','error');
                                return;
                            }if(item.nutritional_status==undefined){
                                swal('','Please Choose Nutritional Status','error');
                                return;
                            }
                            var preg;
                            if(item.preg==1){
preg=1;
                            ...
                            var detail={preg:preg,user_id:user_id,facility_id:facility_id,patient_id:patient.patient_id,visit_id:patient.account_id,
                                action_taken:item.action_taken,
                                nutritional_status:item.nutritional_status,
                                hiv_status:item.hiv_status,
                                description:item.description,
                            }
                            $http.post('/api/Save_nutritional_consultations',detail).then(function(data) {
 ...
 $scope.Suppliments_Registry = function(item) {
                            if(item==undefined){
                                swal('','Please Fill All required Field','error');
                                return;
                            }

                            $http.post('/api/Suppliments_Registry',{suppliment_name:item.suppliment_name}).then(function(data) {
                                if(data.data.status==1){
                                    swal('',data.data.msg,'success');
                                    $scope.Suppliments_list();
                                    $('#suppliment_name').val('');
                                }
                                else{
                                    swal('',data.data.msg,'error');
...
$scope.Save_nutritional_supplimentray = function(item, patient) {
                            if(item==undefined){
                                swal('','Please Fill All required Field','error');
                                return;
                            }
    var detail={user_id:user_id,facility_id:facility_id,patient_id:patient.patient_id,visit_id:patient.account_id,
        suppliment_id:item.suppliment_id,
        description:item.description,
    }
                            $http.post('/api/Save_nutritional_supplimentray',detail).then(function(data) {
                                if(data.data.status==1){
                                    swal('',data.data.msg,'success');
                        ...
                        $scope.Save_client_nutritional_status = function(item, patient) {
                            ...
                            if(item.status==undefined){
                                swal('','Please Fill Client Nutritional Status','error');
                                return;
                            }
    var detail={user_id:user_id,facility_id:facility_id,patient_id:patient.patient_id,visit_id:patient.account_id,
        status:item.status,
        ...
        description:item.description,
                            ...
                            $http.post('/api/Save_client_nutritional_status',detail).then(function(data) {
                                if(data.data.status==1){
                                    swal('',data.data.msg,'success');
                        ...
                        $scope.Suppliments_list = function() {
                            $http.get('/api/Suppliments_list', item).then(function (data) {
                                $scope.suppliments = data.data;
                            });
                        }
                        $scope.Suppliments_list();
                        //chief complaints
                    ...
                    templateUrl: '/views/modules/clinic/Nutrition/NutritionConsultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
            ...
            $scope.Nutrition_mtuha=function (item) {

               var data={facility_id:facility_id,start_date:item.start_date,end_date:item.end_date}
                $http.post('/api/Nutrition_mtuha',data).then(function(data) {
                    $scope.nutrition=data.data;

                });
            }
            $scope.PrintContent_nutri=function () {

                //location.reload();
                var DocumentContainer = document.getElementById('divtoprint_nut');
