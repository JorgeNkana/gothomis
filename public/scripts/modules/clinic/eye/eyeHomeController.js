...
(function () {
    ...
    function eyeHomeController($scope,$state,$http,$rootScope,$uibModal,$mdDialog) {
        var facility_id = $rootScope.currentUser.facility_id;
        var user_id = $rootScope.currentUser.id;
        angular.element(document).ready(function () {
            $http.post('/api/incomingEyePatients',{"facility_id":facility_id,"dept_id":10}).then(function (data) {
        ...
        $scope.showSearch = function (searchKey) {
            $http.post('/api/eyeAll',{"searchKey":searchKey,"facility_id":facility_id,"dept_id":10}).then(function (data) {
                eyeData = data.data;
            });
            return eyeData;
        }
        ...
        $scope.getEyeModal = function (item) {
            $http.post('/api/updateClinicClient',{"dept_id":item.dept_id,"visit_id":item.visit_id,"user_id":user_id}).then(function (data) {
			...
			$http.post('/api/getStores',{facility_id:$rootScope.currentUser.facility_id}).then(function (data) {
                    ...
                    $scope.snelleen = function (item, patient, chart, category) {
                        if (item == true) {
                            for (var i = 0; i < $scope.snellenTemp.length; i++)
                                if ($scope.snellenTemp[i].name == chart.name) {
                                    swal(chart.name + " already in your order list!");
                                    return;
                                }
                            $scope.snellenTemp.push({
                                "admission_id": '',
                    ...
                    $scope.eyeExamination = function (item, patient) {
                        for (var i = 0; i < $scope.eyeFindings.length; i++)
                            if ($scope.eyeFindings[i].id == item.id) {
                                swal(item.description + " already in your order list!");
                                return;
                            }
                        $scope.eyeFindings.push({
                            "admission_id": '',
                            "patient_id": patient.patient_id,
                    ...
                    $scope.saveSnellen = function (objectData,va) {
                        var otherData = {
                            "admission_id": '',
                        ...
                        for (var i = 0; i < objectData.length; i++) {
                            objectData[i]['sub_category'] = $scope.eyeSide[0].category;
                        }

                        var eyeData = {details:otherData,findings:objectData,otherData:va,eye_side:$scope.eyeSide[0].category};
                        $http.post('/api/eyeFindings',eyeData).then(function (data) {

                        });
                        swal($scope.eyeSide[0].category + ' charts data saved', '', 'success');
                        $scope.snellenTemp = [];
                        //$scope.eyeSide = [];
                        $("#hand_movement").val('');
                        $("#pl").val('');
			...
			$scope.saveRefraction = function (item) {
                             var otherData = {
                            "admission_id": '',
                            "patient_id": patient_id,
                            "visit_date_id": account_id,
                        ...
                        var  refractionData = {details:otherData,refraData:item,category:$scope.eyeSide[0].category};
                        $http.post('/api/eyeRefractionFindings', refractionData).then(function (data) {
                        if(data.data.status == 1){
                           swal('',data.data.msg,'success');
                        }else {
                            swal('There was an error during saving your data','Make sure you have selected the eye side','error');
                        }
                        });
                        $("#sphere").val('');
                        $("#cylinder").val('');
                    ...
                    $scope.saveExamination = function (objectData) {
                        ...
                        var eyeData = {details:otherData,findings:objectData,eye_side:$scope.eyeSide[0].category};
                    ...
                    $scope.savePastEye = function (patient,item) {
                        var pastEye = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,
                            "user_id": user_id,
                            "past_ocular_history":item.past_eye_history
                        };
                    }
                    ...
                    $scope.getPatientReport = function(item) {
                        ...
                        $http.post('/api/eyeExaminationReport', {
                            ...
                            $scope.refraction = data.data[0];
                           $scope.va1 = data.data[2];
                           $scope.va2 = data.data[1];
                           $scope.eye_exam = data.data[3];
                        ...
                        $scope.orders = function(item, isChecked, patient) {
                            ...
                            if(sub_category_name=='chf'){
                                $http.post('api/chfCheckBills',{patient_id:patient_id,account_id:account_id}).then(function (data) {
                                    $scope.totalCHFBils = data.data[0][0].sub_total;
...
//Provisional , differential and confirmed diagnosis
                    ...
                    $scope.diagnosisTemp = [];
                    $scope.addDiagnosis = function (item,patient,status) {
    if(patient.patient_id == null)
    { swal("Ooops!! no Patient selected","Please search and select patient first..");
    return;
    }
    $scope.diagnosisTemp.push({"admission_id":'',
    "patient_id":patient.patient_id,"visit_date_id":patient.account_id,
    "facility_id":facility_id,"user_id":user_id,"diagnosis_description_id":item.id,
    "code":item.code,"description":item.description,"status":status
    });
    }
                    ...
                    $scope.saveDiagnosis = function(objectData) {
                        ...
                        $http.post('/api/postDiagnosis', objectData).then(function(data) {
                            ...
                            var TallyRegister = {attempt:0, load: function(){
                                ...
                                $http.post('/api/countClinicDiagnosis',{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender,concepts:confirmedDiagnoses, clinic_id: 10}).then(function(data){},function(data){if(TallyRegister.attempt < 5) TallyRegister.load();});
                            }}
                            TallyRegister.load();
                    ...
                    $scope.deceased = function(item, corpse) {
                        ...
                        var deceased = {
                            ...
                            "last_name": item.last_name,"residence_id":item.residence_id,
                            "patient_id": item.patient_id,
                            "user_id": user_id,
                            "facility_id": facility_id,
        ...
        $scope.tallyAttendance = function(attendance){
            ...
            var TallyRegister = {attempt:0, load: function(){
                TallyRegister.attempt++;
                $http.post('/api/'+(attendance.toLowerCase() == 'new' ? 'countNewAttendance' : 'countReattendance'),{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender, clinic_id:10}).then(function(data){
