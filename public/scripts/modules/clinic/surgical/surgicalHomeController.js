...
(function() {
    'use strict';
    angular.module('authApp').controller('surgicalHomeController', surgicalHomeController);

    function surgicalHomeController($scope, $state, $http, $rootScope, $uibModal,$mdDialog) {
        var facility_id = $rootScope.currentUser.facility_id;
        var user_id = $rootScope.currentUser.id;
        angular.element(document).ready(function() {
            $http.post('/api/incomingSurgicalPatients', {
                "facility_id": facility_id,
                "dept_id": 27
            }).then(function(data) {
                $scope.surgicalPatients = data.data;
            });
        });
        ...
        $scope.showSearch = function(searchKey) {
            $http.post('/api/surgicalAll', {
                "searchKey": searchKey,
                "facility_id": facility_id,
                "dept_id": 27
            }).then(function(data) {
                surgicalData = data.data;
            });
            return surgicalData;
        }
        $scope.checkAttendance = function(patient){
            ...
            $http.post('/api/checkPatientAttendance',{patient_id:patient.patient_id,
                ...
                if(data.data == 1){
                    ...
                    $scope.getSurgicalModal(patient);
                }
            });
        }
        ...
        $scope.getSurgicalModal = function(item) {
...
//Provisional , differential and confirmed diagnosis
                    ...
                    $scope.addProv = function(item, patient, status) {
                        ...
                        $scope.diagnosisTemp.push({
                            ...
                            "diagnosis_description_id": item.id,"code":item.code,
                    ...
                    $scope.saveDiagnosis = function(objectData) {
                        ...
                        $http.post('/api/postDiagnosis', objectData).then(function(data) {
                            ...
                            var TallyRegister = {attempt:0, load: function(){
                                ...
                                $http.post('/api/countClinicDiagnosis',{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender,concepts:confirmedDiagnoses, clinic_id: 27}).then(function(data){},function(data){if(TallyRegister.attempt < 5) TallyRegister.load();});
                            }}
                            TallyRegister.load();
                    ...
                    $scope.deceased = function(item, corpse) {
                        ...
                        var deceased = {
                            ...
                            "patient_id": item.patient_id,"residence_id":item.residence_id,
                            "user_id": user_id,
                            "facility_id": facility_id,
                ...
                templateUrl: '/views/modules/clinic/surgical/surgical.html',
                parent: angular.element(document.body),
                clickOutsideToClose: false,
                fullscreen: true,
            });
        ...
        $scope.tallyAttendance = function(attendance){
            ...
            var TallyRegister = {attempt:0, load: function(){
                TallyRegister.attempt++;
                $http.post('/api/'+(attendance.toLowerCase() == 'new' ? 'countNewAttendance' : 'countReattendance'),{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender, clinic_id:27}).then(function(data){
