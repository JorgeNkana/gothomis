...
(function() {
    ...
    function dentalHomeController($scope, $state, $http, $rootScope, $uibModal,$mdDialog) {
        var facility_id = $rootScope.currentUser.facility_id;
        var user_id = $rootScope.currentUser.id;
        angular.element(document).ready(function() {
            $http.post('/api/incomingDentalPatients', {
        ...
        $scope.getDentalModal = function(patient) {
...
//posting from history and examinations
                    ...
                    $scope.addProv = function (item,patient,status) {
                        ...
                        if(patient.patient_id == null){ swal("Ooops!! no Patient selected","Please search and select patient first.."); return; }
                        $scope.diagnosisTemp.push({"admission_id":'',"patient_id":patient.patient_id,"visit_date_id":patient.account_id,"facility_id":facility_id,"user_id":user_id,"diagnosis_description_id":item.id,"code":item.code,"description":item.description,"status":status});
                    }
                    ...
                    $scope.addProcedure = function (patient,item,qty) {
                        ...
                        $scope.procedures.push({"payment_filter":filter,"admission_id":'',"facility_id":facility_id,"item_type_id":item.item_type_id,"item_price_id":item.price_id,"quantity":qty,"status_id":status_id,
                            "account_number_id":patient.account_id,"patient_id":patient.patient_id,"user_id":user_id,"item_id":item.item_id,"item_name":item.item_name
                        });
                    }
                    ...
                    $scope.deceased = function (item,corpse) {
                        ...
                        var deceased = {"patient_id":item.patient_id,"residence_id":item.residence_id,"user_id":user_id,"facility_id":facility_id,"immediate_cause":corpse.immediate_cause,"underlying_cause":corpse.underlying_cause};
