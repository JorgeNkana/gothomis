...
(function() {
    ...
    function ctcController($http, $auth, $rootScope,$state,$location,$scope,$uibModal,toastr,$filter,$mdDialog) {
  ...
  $scope.getClinicAttendanceForPatient= function (ctcQues) {
        ...
        $scope.AdmissionNotes="";
            ...
            $http.post('/api/incomingCtcPatients', {
                "facility_id": facility_id,
                "dept_id": 8
            }).then(function(data) {
                $scope.ctcIncomings = data.data;
            });
        });
		
		var psychData = [];
        $scope.showSearch = function(searchKey) {
            $http.post('/api/psychAll', {
                "searchKey": searchKey,
                "facility_id": facility_id,
                "dept_id": 8
            }).then(function(data) {
		...
		$scope.tallyAttendance = function(attendance){
            ...
            var TallyRegister = {attempt:0, load: function(){
                ...
                $http.post('/api/'+(attendance.toLowerCase() == 'new' ? 'countNewAttendance' : 'countReattendance'),{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender, clinic_id:9}).then(function(data){
		 ...
		 $scope.getCtcDataModal = function(patient) {
            $http.post('/api/updateClinicClient', {
            ...
            $mdDialog.show({
                controller: function ($scope) {
                    ...
                    $scope.savePastHistory = function (patient,item) {
                        var past = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,

                            "user_id": user_id,
                            "other_past_medicals":item.other_past_medicals,
                            "past_surgical":item.past_surgical
                        };
                        $http.post('/api/postPastMedicalSurgical',past).then(function (data) {

                        });
                        swal("Medical and surgical history data successfully Saved!", "", "success");
                        $("#other_past_medicals").val('');
                        $("#past_surgical").val('');
                    }
                    $scope.savePastDental = function (patient,item) {
                        var pastDental = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,
                            "user_id": user_id,
                            "past_dental_history":item.past_dental_history
                        };
                        $http.post('/api/postPastDental',pastDental).then(function (data) {

                        });
                        swal("Past dental data successfully Saved!", "", "success");
                        $("#past_dental_history").val('');
                    };   

					$scope.savePastPsych = function (patient,past_psych_history) {
                        var PastPsych = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,
                            "user_id": user_id,
                            "past_psychiatric_history":past_psych_history
                        };
                        $http.post('/api/postPastPsych',PastPsych).then(function (data) {

                        });
                        swal("Past Psychiatric History was  successfully Saved!", "", "success");
                        $("#past_dental_history").val('');
                    };
					
					$scope.saveForensicHistory = function (patient,item) {
                        var PastPsych = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,
                            "user_id": user_id,
                            "forensic_history":item
                        };
                        $http.post('/api/forensicHistory',PastPsych).then(function (data) {

                        });
                        swal("Forensic History was  successfully Saved!", "", "success");
                        $("#past_dental_history").val('');
                    };
					
					$scope.saveFamilyHistory = function (patient,item) {
						if(angular.isDefined(item)==false){
						   sweetAlert("Write Family and Social History", "", "error");	
						}
                        var familyHist = {"patient_id": patient.patient_id,
                            "visit_date_id": patient.account_id,
                            "facility_id": facility_id,
                            "user_id": user_id,
                        ...
                        $http.post('/api/familyHistory',familyHist).then(function (data) {
                    ...
                    $scope.saveSocialCommunity = function (patient,item) {
...
//Previous History
					...
					$scope.saveNutritionalStatus=function (selectedPatient) {
						if(angular.isDefined($scope.code)==false){
							 return sweetAlert("Please Enter related value for nutrition", "", "error");
						}
					var nutrition_status=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="nutrition_status";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	nutrition_status:nutrition_status,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
					...
					$scope.saveNutritionalSupplement=function (selectedPatient) {
						if(angular.isDefined($scope.code)==false){
							 return sweetAlert("Please Enter related value for nutrition", "", "error");
						}
					var nutritional_supplement=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="nutritional_supplement";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	nutritional_supplement:nutritional_supplement,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
                    ...
                    $scope.getClinicCodes = function (selectedPatient) {
                      var dataToPost={patient_id:selectedPatient.patient_id};						
					 $http.post('/api/getCodesPerCTC',dataToPost).then(function (data) {
                            $scope.visitTypeCodes = data.data[0];
                            $scope.sysmtomsCodes = data.data[1];
                            $scope.functionalStatusCodes = data.data[2];
							$scope.familyPlans = data.data[3];
                            $scope.visitHostories = data.data[4];
							$scope.status_nutritionals = data.data[5];
							$scope.nutrition_suplements = data.data[6];
							$scope.refferals = data.data[7];
							$scope.followUps = data.data[8];
							/** 
                            
                            $scope.screeningTBCodes = data.data[4];
                            $scope.TBRxCodes = data.data[5];
                            $scope.ARVStatusCodes = data.data[6];
                            $scope.ARVSReasonCodes = data.data[7];
							**/
					...
					$scope.saveAnc = function (selectedPatient,anc) {
						if(angular.isDefined(anc)==false){
							 return sweetAlert("Please Enter related value", "", "error");
						}
					var anc=anc;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="anc";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,anc:anc,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
					...
					$scope.savefamilyPlan = function (selectedPatient) {
						if(angular.isDefined($scope.code)==false){
							 return sweetAlert("Please Enter related value", "", "error");
						}
					var pregnant=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="pregnant";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	pregnant:pregnant,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
						...
						$scope.saveCtcRefferal = function (selectedPatient) {
						if(angular.isDefined($scope.code)==false){
							 return sweetAlert("Please Enter related value", "", "error");
						}
					var ctc_refferal=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="ctc_refferal";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	ctc_refferal:ctc_refferal,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
					...
					$scope.saveFollowUp = function (selectedPatient) {
					if(angular.isDefined($scope.code)==false){
						 return sweetAlert("Please Enter follow up status", "", "error");
						}
					var follow_up=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="follow_up";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	follow_up:follow_up,user_id:user_id,column:affectedColmn};						
					 $http.post('/api/savePatientClinic',dataToPost).then(function (data) {
					...
					$scope.saveEdd = function (selectedPatient,edd_date) {
					var pregnant=moment(edd_date).format('YYYY-MM-DD');
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="pregnant";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,pregnant:pregnant,user_id:user_id,column:affectedColmn};						
					...
					$scope.getSelectedCode = function (item) { 
					$scope.code=item;
						};
						
					$scope.getSelectedAnswer = function (answer) {
						$scope.showOption=0;
					  if(answer=='N'){
					...
					$scope.saveSignsType = function (selectedPatient) { 
					var signs_symptoms_code=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="signs_symptoms_code";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,signs_symptoms_code:signs_symptoms_code,user_id:user_id,column:affectedColmn};
					...
					$scope.saveFunctionalType = function (selectedPatient) { 
					var functional_status_code=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="functional_status_code";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,	functional_status_code:functional_status_code,user_id:user_id,column:affectedColmn};
					...
					$scope.saveVisitType = function (selectedPatient) { 
					var visit_type_code=$scope.code.code;
					var visit_id=selectedPatient.visit_id;
					var patient_id=selectedPatient.patient_id;
					var affectedColmn="visit_type_code";
					var dataToPost={patient_id:patient_id,visit_date_id:visit_id,visit_type_code:visit_type_code,user_id:user_id,column:affectedColmn};
                    ...
                    $scope.saveInvestigation = function(item) {
                        ...
                        var details = {"admission_id":'',"patient_id": patient_id,"visit_date_id": account_id,
                            "user_id": user_id,"facility_id": facility_id,"requesting_department_id":9};
...
//posting from history and examinations
                    ...
                    $scope.savePastDental = function (patient,details) {
                        ...
                        swal('Past Pyschiatric history data successfully saved','','success');
                        $("#past_dental_history").val('');
                    }
                        ...
                        $http.post('/api/postDiagnosis',objectData).then(function (data) {
                            ...
                            var TallyRegister = {attempt:0, load: function(){
                                if(confirmedDiagnoses.length == 0)
                                    return;
                                TallyRegister.attempt++;
                                $http.post('/api/countClinicDiagnosis',{facility_id:facility_id, dob: $scope.selectedPatient.dob,gender: $scope.selectedPatient.gender,concepts:confirmedDiagnoses, clinic_id: 9}).then(function(data){},function(data){if(TallyRegister.attempt < 5) TallyRegister.load();});
                            }}
                            TallyRegister.load();
                        });
                        $scope.diagnosisTemp =[];
                    ...
                    $scope.internalTransfer = function (clinic,patient) {
                        var patientDetails = {"patient_id":patient.patient_id,"main_category_id":patient.main_category_id,"bill_id":patient.bill_id,"sender_clinic_id":9,"first_name":patient.first_name,"middle_name":patient.middle_name,"last_name":patient.last_name,
                    ...
                    $scope.addSupplies = function (patient,qty,item) {
                        ...
                        $http.post('/api/balanceCheck',{"main_category_id":main_category,"item_id":item.item_id,"facility_id":facility_id, "user_id": user_id}).then(function (data) {
                            ...
                            if(balance02.length >0 && balance02[0].balance>=quantity){
                                $scope.supplies.push({"visit_id":patient.account_id,"out_of_stock":'',"payment_filter":filter,"admission_id":'',"facility_id":facility_id,"item_type_id":item.item_type_id,"item_price_id":item.price_id,"quantity":qty,"status_id":status_id,
                                    "account_number_id":patient.account_id,"patient_id":patient.patient_id,"user_id":user_id,"item_id":item.item_id,"item_name":item.item_name
                                });
                            }
                                ...
                                if(conf== true){
                                    ...
                                    $scope.suppliesOS.push({"visit_id":patient.account_id,"out_of_stock":'OS',"payment_filter":filter,"admission_id":'',"facility_id":facility_id,"item_type_id":item.item_type_id,"item_price_id":item.price_id,"quantity":qty,"status_id":status_id,
                                        "account_number_id":patient.account_id,"patient_id":patient.patient_id,"user_id":user_id,"item_id":item.item_id,"item_name":item.item_name
                                    });
                                    swal("Item added under Out of Stock category","","success");
                                }else {
                ...
                templateUrl: '/views/modules/clinic/ctc/ctc.html',
                parent: angular.element(document.body),
                clickOutsideToClose: false,
                fullscreen: true,
            });
