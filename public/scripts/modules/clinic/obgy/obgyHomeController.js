...
(function() {
    'use strict';
    angular.module('authApp').controller('obgyHomeController', obgyHomeController);

    function obgyHomeController($scope, $state, $http, $rootScope, $uibModal,$mdDialog) {
        var facility_id = $rootScope.currentUser.facility_id;
        var user_id = $rootScope.currentUser.id;
        angular.element(document).ready(function() {
            $http.post('/api/incomingObgyPatients', {
                "facility_id": facility_id,
                "dept_id": 26
            }).then(function(data) {
                $scope.obgyPatients = data.data;
            });
        });
        var obgyData = [];
        $scope.showSearch = function(searchKey) {
            $http.post('/api/obgyAll', {
                "searchKey": searchKey,
                "facility_id": facility_id,
                "dept_id": 26
            }).then(function(data) {
        ...
        $scope.getObgyModal = function(item) {
...
//Provisional , differential and confirmed diagnosis
                ...
                templateUrl: '/views/modules/clinic/obgy/obgy.html',
                parent: angular.element(document.body),
                scope: $scope,
                clickOutsideToClose: false,
                fullscreen: true,
