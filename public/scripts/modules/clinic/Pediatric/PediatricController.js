...
(function () {
    ...
    app.controller('PediatricController',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
...
//console.log(selectedPatient)
                            $http.get('/api/patientAge/'+selectedPatient.dob ).then(function(data) {
                                ...
                                var modalInstance = $uibModal.open({
                                    templateUrl: '/views/modules/clinic/VCT/Pitc.html',
                                    size: 'lg',
                                    animation: true,
                                    controller: 'Vct_data',
            ...
            $scope.getDTC = function(item) {
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        $scope.SaveDTC=function (item,patient) {
                            if(item==undefined){
                                swal('','Nothing To save , Please Fill Either Of The Fields Given','error') ;
                                return;
                            }
                            var dtcs={
                                dct_duration:item.dct_duration,
                                dir_duration:item.dir_duration,
                                dtc_unit:item.dtc_unit,
                                fever:item.fever,
                                intravesel_water:item.intravesel_water,
                                ors_in:item.ors_in,
                                ors_out:item.ors_out,
                                other_sign:item.other_sign,
                                other_treatment:item.other_treatment,
                                output:item.output,
                                stool_blood:item.stool_blood,
                                vomiting:item.vomiting,
                                water_sugar_loss:item.water_sugar_loss,
                                zink_in:item.zink_in,
                                zink_out:item.zink_out,
                                visit_id:patient.account_id,
                                patient_id:patient.patient_id,
                                user_id:user_id,
                                facility_id:facility_id,
                            ...
                            console.log(dtcs)
                            $http.post('/api/postDTC',dtcs).then(function(data) {
                                $scope.dtc = data.data;
                    ...
                    templateUrl: '/views/modules/clinic/Pediatric/Dtc.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
            };
            $scope.DTC_mtuha = function() {
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        $scope.mtuhaDTC = function(item) {
                            $http.post('/api/mtuhaDTC', {start:item.start_date,end:item.end_date,"facility_id": facility_id}).then(function(data) {
                                $scope.dct = data.data;

                            });
                        }
                        $scope.dtc_printer=function () {

                            //location.reload();
                            var DocumentContainer = document.getElementById('dtc_print');
