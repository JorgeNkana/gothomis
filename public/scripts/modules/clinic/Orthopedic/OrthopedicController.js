...
(function () {
    ...
    app.controller('OrthopedicController',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
...
//console.log($scope.resdatas);
            ...
            $scope.getConsultationModal = function(item) {
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        $scope.savepast_orthopedic = function(patient, examData) {
                            
                            if (examData == null) {
                                swal('Please write Past Orthopedic history for this patient first', '', 'error');
                                return;
                            }
                            var past_orthopedic = {
                                "admission_id": '',
                                ...
                                "user_id": user_id,
                                "facility_id": facility_id,
                                "past_orthopedic": examData

                            }

                            $http.post('/api/postpast_orthopedic', past_orthopedic).then(function(data){
                                var msg=data.data.msg;
                                var status=data.data.status;
                                if(status==1){
                                    swal('Past Orthopedic History', 'For this patient saved', 'success');

                                    $('#past_orthopedic').val('');

                                }
                                else{
                                    swal('Past Orthopedic History', msg, 'error');
 ...
 $scope.getTodaysVitals = function (item) {
 		...
 		$scope.getPatientVitals = function (item) {
                    ...
                    templateUrl: '/views/modules/clinic/Orthopedic/OrthopedicConsultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
