...
(function () {
    ...
    app.controller('diabeticClinicController',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
        function ($scope,$http,$rootScope,$state,$uibModal,$window,$mdDialog) {
            ...
            $scope.ClinicQueue = function () {
                $http.get('/api/searchClinicpatientQueue/' + facility_id).then(function (data) {
                    $scope.patientData1 = data.data[14];
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        $scope.savepast_ent = function(patient, examData) {
                            ...
                            if (examData == null) {
                                swal('Please write Past Diabetic history for this patient first', '', 'error');
                                return;
                            }
                            var past_ent = {
                                "admission_id": '',
                                ...
                                "user_id": user_id,
                                "facility_id": facility_id,
                                "past_diabetic": examData

                            }

                            $http.post('/api/savepast_diabetic', past_ent).then(function(data){
                                var msg=data.data.msg;
                                var status=data.data.status;
                                if(status==1){
                                    swal('Past Diabetic History', 'For this patient saved', 'success');

                                    $('#past_ent').val('');

                                }
                                else{
                                    swal('Past Diabetic History', msg, 'error');
                    ...
                    templateUrl: '/views/modules/clinic/diabetic/diabetic_consultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
