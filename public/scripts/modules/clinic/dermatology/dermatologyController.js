...
(function () {
    ...
    app.controller('dermatologyController',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
            ...
            $scope.getConsultationModal = function(item) {
                ...
                $mdDialog.show({
                    controller: function ($scope) {
                        ...
                        //past ent history
                        $scope.savepast_ent = function(patient, examData) {

                            if (examData == null) {
                                swal('Please write Past Dermatology history for this patient first', '', 'error');
                                return;
                            }
                            var past_ent = {
                                "admission_id": '',
                                ...
                                "user_id": user_id,
                                "facility_id": facility_id,
                                "past_dermatology": examData

                            }

                            $http.post('/api/savepast_dermatology', past_ent).then(function(data){
                                var msg=data.data.msg;
                                var status=data.data.status;
                                if(status==1){
                                    swal('Past Dermatology History', 'For this patient saved', 'success');

                                    $('#past_ent').val('');

                                }
                                else{
                                    swal('Past Dermatology History', msg, 'error');
                        ...
                        $scope.getPatientReport = function(item) {
                            ...
                            $http.post('/api/DermatologyHistory', {
                    ...
                    templateUrl: '/views/modules/clinic/dermatology/dematologyConsultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
