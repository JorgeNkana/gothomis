...
(function () {
    ...
    app.controller('Urology_Controller',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
 ...
 $scope.getTodaysVitals = function (item) {
 		...
 		$scope.getPatientVitals = function (item) {
                      $http.post('/api/patientVitals',{patient_id:patient_id,account_id:account_id,time_taken:item.created_at}).then(function (data) {
                        ...
                        //past ent history
                        $scope.savepast_urology = function(patient, examData) {

                            if (examData == null) {
                                swal('Please write Past Urology history for this patient first', '', 'error');
                                return;
                            }
                            var past_ent = {
                                "admission_id": '',
                                ...
                                "user_id": user_id,
                                "facility_id": facility_id,
                                "savepast_urology": examData

                            }

                            $http.post('/api/savepast_urology', past_ent).then(function(data){
                                var msg=data.data.msg;
                                var status=data.data.status;
                                if(status==1){
                                    swal('Past Urology History', 'For this patient saved', 'success');

                                    $('#past_ent').val('');

                                }
                                else{
                                    swal('Past Urology History', msg, 'error');
...
$http.post('/api/uroloHistory', {
                    ...
                    templateUrl: '/views/modules/clinic/Urology/UrologyConsultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
