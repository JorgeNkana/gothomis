...
(function () {
    ...
    app.controller('Ent_Controller',['$scope','$http','$rootScope','$state','$uibModal','$window','$mdDialog',
 		...
 		$scope.getPatientVitals = function (item) {
                      $http.post('/api/patientVitals',{patient_id:patient_id,account_id:account_id,time_taken:item.created_at}).then(function (data) {
                        ...
                        $scope.savepast_ent = function(patient, examData) {
                            ...
                            $http.post('/api/savepast_ent', past_ent).then(function(data){
                                var msg=data.data.msg;
                                var status=data.data.status;
                                if(status==1){
                                    swal('Past ENT History', 'For this patient saved', 'success');

                                    $('#past_ent').val('');

                                }
                                else{
                                    swal('Past ENT History', msg, 'error');
...
$http.post('/api/EntHistory', {
                    ...
                    templateUrl: '/views/modules/clinic/Ent/EntConsultation.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose: false,
                    fullscreen: true,
                });
