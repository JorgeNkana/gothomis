...
(function () {
    ...
    app.controller('TbController',['$scope','$http','$rootScope','$state','$window','$mdDialog','$uibModal',
        function ($scope,$http,$rootScope,$state,$window,$mdDialog,$uibModal) {
...
//console.log($scope.resdatas);
            ...
            $scope.checkAttendance = function (patient) {
                ...
                $http.post('/api/checkPatientAttendance', {
  ...
  $http.post('/api/getpatient_tb_history', {
 ...
 $scope.$scope.getTodaysVitals = function (item) {
 		...
 		$scope.getPatientVitals = function (item) {
            ...
            $scope.tallyAttendance = function (attendance) {
                ...
                var TallyRegister = {
                    attempt: 0, load: function () {
                        TallyRegister.attempt++;
                        $http.post('/api/' + (attendance.toLowerCase() == 'new' ? 'countNewAttendance' : 'countReattendance'), {
                            facility_id: facility_id,
                            dob: $scope.selectedPatient.dob,
                            gender: $scope.selectedPatient.gender,
                            clinic_id: 1
                            ...
                            var Tally = {
                                attempt: 0, load: function (patient_id) {
                                    Tally.attempt++;
                                    $http.post('/api/tallied', {patient_id: patient_id}).then(function (data) {
                                    }, function (data) {
                                        if (Tally.attempt < 5) Tally.load(patient_id);
                                    });
                                }
                            };
            ...
            $scope.Tb_mtuha=function (date) {
            var rch_child_attendance = {facility_id: facility_id, start_date: date.start_date, end_date:date.end_date}
            $http.post('/api/Tb_mtuha', rch_child_attendance).then(function (data) {
                $scope.attendances = data.data;

            });
        }
            $scope.PrintContent_tb=function () {

                //location.reload();
                var DocumentContainer = document.getElementById('divtoprint_tb');
