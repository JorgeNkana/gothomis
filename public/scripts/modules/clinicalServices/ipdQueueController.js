...
(function () {
    ...
    function ipdQueueController($scope,$state,$http,$rootScope,$uibModal, $mdDialog) {
	...
	$scope.getByWard = function (item) {
            if(item.ward_id =='all'){
                ...
                $http.post('/api/filterByWards',{ward_id:item.ward_id}).then(function (data) {
                    $scope.inpatients = data.data;
                });
            }
        }
        ...
        $scope.getIpdModal = function (item) {
			...
			$http.post('/api/getStores',{facility_id:$rootScope.currentUser.facility_id}).then(function (data) {
                    ...
                    $scope.doctor_details = $rootScope.currentUser;
                    $scope.toto = function () {
                        var  total = 0;
                    ...
                    $http.get('/api/getUsermenu/'+user_id ).then(function(data) {
                        ...
                        $http.post('/api/getPatientAdmissionInfo',{patient_id:patient_id,visit_id:account_id}).then(function (data) {
                       $scope.patientAdmissionInfo = data.data;
                    });
                    });
                    $scope.generateDischargeForm = function (item) {
                        var _data = {patient_id:item.patient_id,visit_id:item.account_id};
                        $http.post('/api/dischargerReport',_data).then(function (data) {
                            $scope.diagnosis = data.data[0];
                            $scope.diagnosisIPD = data.data[1];
                            $scope.medications = data.data[2];
                            $scope.procedures = data.data[3];
                            $scope.investigations = data.data[4];
                        });
                    ...
                    $scope.dischargePatient = function (patient,summary) {

                        $http.post('/api/dischargePatient',{user_id:user_id,"discharge_summary":summary,"account_id":patient.account_id,"patient_id":patient.patient_id,"facility_id":facility_id}).then(function (data) {
                            if(data.data.status==0){
                                swal('',data.data.data,'error')
                            }
                            else{
                                $scope.printDischarge();
                                swal('',data.data.data,'success')
                            }
                        });
                    }
                                ...
                                $http.post('api/chfCheckBills',{patient_id:patient_id,account_id:account_id}).then(function (data) {
                                    ...
                                    if (difference >0 && using==1){
                                        ...
                                        $scope.investigationOrders.push({
                                            ...
                                            "requesting_department_id":5,"admission_id":patient.admission_id,
                                            "facility_id": facility_id,
                    ...
                    $scope.saveInvestigation = function(item) {
                        ...
                        var details = {"admission_id":admission_id,"patient_id": patient_id,"visit_date_id": account_id,
...
//posting from history and examinations
                    ...
                    $scope.saveHpi = function (item,patient) {
                        var hpi = {"admission_id": patient.admission_id,
                    ...
                    $scope.saveSystemicSummary = function(patient, examData,system) {
                        if (examData == null) {
                            ...
                            return; }
                        var summary_examz = {
                            "admission_id": patient.admission_id,"patient_id": patient.patient_id,
                    ...
                    $scope.addProv = function (item,patient,status) {
                        ...
                        if(patient.patient_id == null){ swal("Ooops!! no Patient selected","Please search and select patient first.."); return; }
                        $scope.diagnosisTemp.push({"admission_id":patient.admission_id,"patient_id":patient.patient_id,"visit_date_id":patient.account_id,"facility_id":facility_id,"user_id":user_id,"diagnosis_description_id":item.id,"code":item.code,"description":item.description,"status":status});
                    }
					...
					$scope.tallyDiagnosis = function(confirmedDiagnoses){
						...
						var TallyRegister = {attempt:0, load: function(){
							...
							TallyRegister.attempt++;
							$http.post('/api/countIPDDiagnosis',data).then(function(data){},function(data){if(TallyRegister.attempt < 5) TallyRegister.load();});
						}};
						TallyRegister.load();
					}
                    //Dispositions
...
$http.get('/api/getUsermenu/'+user_id).then(function(cardTitle){
 ...
 $scope.refdata=data.data.data;
  ...
  swal($scope.msg,"","error");
                    ...
                    $scope.getNotes = function (patient) {
                        $http.post('/api/getNotes',{"patient_id":patient_id}).then(function (data) {
 ...
 $scope.checkDosage = function(item_id, patient_id,visit_date_id) {
                    ...
                    $scope.addProcedure = function (patient,item,qty) {
                        ...
                        $scope.procedures.push({"payment_filter":filter,"admission_id":patient.admission_id,"facility_id":facility_id,"item_type_id":item.item_type_id,"item_price_id":item.price_id,"quantity":qty,"status_id":status_id,
                            "account_number_id":patient.account_id,"patient_id":patient.patient_id,"user_id":user_id,"item_id":item.item_id,"item_name":item.item_name
                        });
                    }
                    ...
                    $scope.saveConservative = function (patient,preData) {
                        var dt = {
                            patient_id:patient.patient_id,
                            admission_id:patient.admission_id,
                            visit_id:patient.account_id,prescriber_id:user_id,conservatives:preData
                        };
                   ...
                   $scope.deceased = function(item, corpse,diag) {
                    ...
                    $scope.printDischarge=function () {

                        var DocumentContainer = document.getElementById('printDischarge');
                        var WindowObject = window.open("", "PrintWindow",
                            "width=900,height=700,top=50,left=450,toolbars=no,scrollbars=no,status=no,resizable=yes");
                        WindowObject.document.title = "printout: GoT-HOMIS";
                        WindowObject.document.writeln(DocumentContainer.innerHTML);
